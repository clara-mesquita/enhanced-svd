...
# CONFIGURAÇÃO - AJUSTE AQUI O NOME DA PASTA
INPUT_FOLDER = "aggregated_6h_datasets"  # Altere para o nome da sua pasta
OUTPUT_FOLDER = "imputation_results_original"
...
# ============================================================================
# Main Processing Pipeline - MODIFICADA
# ============================================================================
...
def process_all_datasets():
    ...
    # Busca todos os arquivos CSV na pasta de entrada
    ...
    with open(REPORT_FILE, 'w', encoding='utf-8') as report:
        ...
        for input_file in sorted(input_files):
            ...
            try:
                ...
                report.write(f"Dataset shape: {df_missing.shape}\n")
                ...
                for method_name, impute_func in imputation_methods.items():
                    try:
                        ...
                        # Para avaliação, usamos os valores originais não-missing como referência
                        # Nos dados reais, você teria que ter os valores originais completos
                        # Se não tiver, podemos apenas salvar os resultados sem avaliação
                        
                        metrics = {
                            'rmse': 0.0,  # Não podemos calcular sem dados originais completos
                            'mae': 0.0,
                            'n_points': int(missing_mask.sum())
                        }
                        
                        report.write(f"{method_name:<20} {'N/A':>12} {'N/A':>12} {metrics['n_points']:>10}\n")
                        ...
                        all_results.append({
                            ...
                            'missing_percentage': missing_mask.sum()/len(df_missing)*100
                        })
                        
                        # Salva o resultado da imputação
                        output_filename = f"{base_name}_{method_name}_imputed.csv"
                        output_path = os.path.join(OUTPUT_FOLDER, output_filename)
                        df_imputed.to_csv(output_path, index=False)
                        
                        # Gera figura (sem dados originais para comparação)
                        fig_name = f"{base_name}_{method_name}.png"
                        fig_path = os.path.join(FIGURES_FOLDER, fig_name)
                        
                        # Visualização simplificada sem dados originais
                        fig, ax = plt.subplots(figsize=(14, 5))
                        missing_mask_vis = (df_missing[VALUE_COL] == MISSING_SENTINEL) | df_missing[VALUE_COL].isna()
                        
                        ax.plot(df_imputed.index, df_imputed[VALUE_COL], 'r-', 
                                label=f'Imputed ({method_name})', alpha=0.8, linewidth=1.2)
                        ax.scatter(df_imputed.index[missing_mask_vis], df_imputed[VALUE_COL][missing_mask_vis], 
                                   c='orange', s=30, label='Imputed Points', zorder=5, alpha=0.6)
                        
                        ax.set_title(f'{base_name} - {method_name}')
                        ax.set_xlabel('Time Index')
                        ax.set_ylabel(VALUE_COL)
                        ax.legend()
                        ax.grid(True, alpha=0.3)
                        ...
                        print(f"Error in {method_name} for {input_file}: {str(e)}")
        ...
        if len(results_df) > 0:
            ...
            report.write("All imputations completed successfully!\n")
            report.write(f"Processed {len(input_files)} files\n")
            report.write(f"Generated {len(results_df)} imputation results\n")
            
            csv_path = os.path.join(OUTPUT_FOLDER, "imputation_summary.csv")
    ...
    print(f"Imputed datasets saved to: {OUTPUT_FOLDER}")
